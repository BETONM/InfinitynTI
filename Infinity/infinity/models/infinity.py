...
class Infinity(nn.Module):
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # ========== [TI] 학습 시 텍스트 임베딩 처리 ==========
            # 참고: 학습 시에는 train.py의 train_one_ep에서 이미 TI 임베딩으로 치환된
            #       kv_compact가 들어오므로, 여기서는 추가 치환 불필요
            #       추론 시에는 autoregressive_infer_cfg에서 처리
            # ====================================================
            # drop cond
            total = 0
            for le in lens:
    ...
    def autoregressive_infer_cfg(
        ...
        kv_compact, lens, cu_seqlens_k, max_seqlen_k = label_B_or_BLT
        # ========== [TI] 추론 시 TI 임베딩 주입 지점 ==========
        # TODO: 여기에 학습된 TI 임베딩을 주입하는 로직 추가
        ...
        # 
        # 만약 여기서 주입해야 한다면:
        # 1. input_ids 정보가 필요 (별도 파라미터로 전달받거나 저장)
        # 2. "A photo of a" 다음의 learned_token 위치 찾기
        # 3. kv_compact[total:total+le]에서 해당 위치에 TI 임베딩 주입
        # ====================================================
