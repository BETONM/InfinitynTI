...
def build_model_optimizer(args, vae_ckpt):
    ...
    # ========== [TI] TI 임베딩 파라미터 초기화 지점 ==========
    # TODO: 여기에 TI 임베딩 파라미터 생성 및 초기화 로직 추가
    # 학습 프롬프트 형식: "A photo of a {initializer_token}" (예: "A photo of a dog")
    # 
    # 구현 예시:
    # 1. 초기화용 프롬프트 생성: init_prompt = f"A photo of a {args.ti_initializer_token}"
    #    (예: "A photo of a dog", "A photo of a cat" 등)
    # 2. 토크나이징: init_tokens = text_tokenizer(init_prompt, ...)
    # 3. 텍스트 인코더로 임베딩 생성: init_embeddings = text_encoder(...).last_hidden_state
    # 4. initializer_token 위치 찾기:
    #    - init_tokens.input_ids에서 initializer_token의 토큰 ID 위치 찾기
    #    - T5는 서브워드 토크나이저이므로 여러 토큰으로 분할될 수 있음
    #    - 예: "dog" -> ["dog"] 또는 ["do", "g"] 등
    # 5. 해당 위치의 임베딩 추출: init_token_embedding = init_embeddings[0, token_positions, :]
    #    - 여러 서브워드인 경우 평균 또는 첫 번째 토큰 사용
    # 6. TI 파라미터 생성: ti_embeddings = nn.Parameter(init_token_embedding.clone())
    # 7. ti_embeddings.requires_grad = True
    # 8. optimizer_ti = torch.optim.AdamW([ti_embeddings], lr=args.ti_lr)
    # =========================================================
    ...
    with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=20, verbose=True) as telling_dont_kill:
        ...
        for it, data in me.log_every(start_it, iters_train, ld_or_itrt, args.log_freq, args.log_every_iter, header):
            ...
            with maybe_record_function('before_train'):
                ...
                # ========== [TI] 텍스트 임베딩 치환 지점 ==========
                # TODO: 여기에 TI 임베딩으로 치환하는 로직 추가
                # 학습 프롬프트 형식: "A photo of a {initializer_token}" (예: "A photo of a dog")
                # 
                # 구현 예시:
                # 1. 각 배치 샘플의 caption이 "A photo of a {token}" 형식이라고 가정
                # 2. input_ids에서 initializer_token 위치 찾기:
                #    - caption을 토크나이징하여 "A photo of a" 다음의 토큰 위치 찾기
                #    - 또는 args.ti_initializer_token을 토크나이징하여 토큰 ID 찾기
                #    - T5 토크나이저는 서브워드로 분할되므로 모든 서브워드 위치 찾기
                #    예: "dog" -> [token_id1, token_id2] (여러 서브워드인 경우)
                # 3. 각 배치 샘플마다 해당 위치의 text_features를 TI 임베딩으로 치환:
                #    for batch_idx in range(B):
                #        for token_pos in token_positions:
                #            text_features[batch_idx, token_pos, :] = ti_embeddings
                # 4. 배치 내 각 샘플의 실제 길이(lens)를 고려하여 마스킹 처리
                #    - lens[batch_idx]를 초과하는 위치는 치환하지 않음
                # ===================================================
