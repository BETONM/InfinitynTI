...
class InfinityTrainer(object):
    def __init__(
        ...
        # ========== [TI] TI 옵티마이저 초기화 지점 ==========
        # TODO: 여기에 TI 옵티마이저를 self에 저장
        # 예시: self.optimizer_ti = optimizer_ti (train.py에서 생성된 것을 전달받아야 함)
        # ===================================================
    ...
    def train_step(
        ...
        # ========== [TI] TI 옵티마이저 스텝 지점 ==========
        # TODO: 여기에 TI 파라미터만 업데이트하는 로직 추가
        # 예시:
        # 1. GPT 모델의 파라미터는 gradient를 zero로 설정하거나 requires_grad=False로 freeze
        # 2. self.optimizer_ti.step() 호출
        # 3. self.optimizer_ti.zero_grad() 호출
        # 주의: GPT 옵티마이저는 이미 backward_clip_step에서 처리되므로, TI만 별도 처리
        # =================================================
        ...
        if stepping:
            ...
            # ========== [TI] TI 옵티마이저 zero_grad 지점 ==========
            # TODO: stepping이 True일 때 TI 옵티마이저도 zero_grad
            # 예시: self.optimizer_ti.zero_grad()
            # ======================================================
    ...
    def state_dict(self):
        ...
        # ========== [TI] TI 임베딩 저장 지점 ==========
        # TODO: 여기에 TI 임베딩을 state에 추가
        # 예시: state['ti_embeddings'] = self.ti_embeddings.state_dict()
        # 예시: state['ti_optimizer'] = self.optimizer_ti.state_dict()
        # ============================================
    ...
    def load_state_dict(self, state, strict=True, skip_vae=False):
        # ========== [TI] TI 임베딩 로드 지점 ==========
        # TODO: 여기에 TI 임베딩을 state에서 로드
        # 예시:
        # if 'ti_embeddings' in state:
        #     self.ti_embeddings.load_state_dict(state['ti_embeddings'])
        # if 'ti_optimizer' in state:
        #     self.optimizer_ti.load_state_dict(state['ti_optimizer'])
        # ============================================
